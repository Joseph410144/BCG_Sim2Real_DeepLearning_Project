...
class BedBCGGenerator:
        ...
        while i < len(rr_intervals):
            ...
            if not is_anomaly:
                ...
                if event_type == 'PVC':
                    ...
                    premature_rr_para = self.beta_in_range(low=0.55, high=0.7, a=3, b=3, size=None)
                    premature_rr = rr_intervals[i] * premature_rr_para
                    compensatory_pause = rr_intervals[i] * (2-premature_rr_para)
    ...
    def generate(self, duration=10):
        ...
        theta, hr_mean, t = self.generate_arrhythmia_profile(duration, mean_bpm=60, anomaly_prob=0.05)
        bcg_pure = self.get_full_bcg_waveform(theta, hr_mean)
        # bcg_pure = self.apply_mattress_transfer_function(bcg_pure) * 3

        # 4. AM 調變
        am_depth = 0.5#self.beta_in_range(low=0.05, high=0.3, a=3, b=3, size=None)
        ...
        # 總雜訊
        total_high_freq_noise = white_noise + vib_noise + movement
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        noisy_bcg = BandPassFilter(signal_=noisy_bcg, lowcut=0.5, highcut=25, step=4, fs=100, padlen=500)
        plt.subplot(211)
        plt.plot(noisy_bcg)
        plt.xticks([])
