...
\small \texttt{josephhwang2400@gmail.com}}
\date{}
...
This work studies that alignment through controlled and interpretable analyses of a deterministic multi-band estimator. Its contributions are: (1) a controlled demonstration of how nonlinear envelope operations expose low-frequency periodicity; (2) a full-dataset decomposition of representation, fusion/scoring, and interval-refinement failures; (3) paired analysis of representation degradation under noise, saturation, and missing samples; and (4) an empirical comparison showing that conventional waveform repair does not reliably restore estimator-relevant cardiac information. An audited ECG reference and verified continuous-source grouping support these analyses. All human data are repeated measurements of one individual; the results characterize these recordings and do not estimate population performance.
...
The evaluated system is a deterministic multi-band BCG heart-rate estimator, hereafter HeartV6. For BCG signal $x(t)$, it uses six overlapping mechanical bands
\begin{equation}
...
\end{tikzpicture}
\caption{Signal path of the evaluated multi-band estimator. The analysis distinguishes representation, fusion/scoring, and interval-refinement stages without altering the implementation.}
\label{fig:pipeline}
\end{figure}
...
Table~\ref{tab:baseline} gives descriptive performance on all 9,006 windows. The multi-band estimator reduced error relative to direct FFT and autocorrelation. These are within-person repeated-recording results, not estimates for unseen individuals.
...
The audited and legacy ECG estimates differed by 1.12~BPM on average, and 2.31\% differed by more than 5~BPM. In legacy group 7, reference correction reduced the apparent estimator MAE from 12.26 to 2.46~BPM, illustrating the importance of label audit before estimator optimization.
...
The reconstruction study provides the central negative result. A filter can reduce sample-wise waveform error without restoring the low-frequency, cross-band, or harmonic structure used by the evaluated estimator. Conversely, interpolation may create a smooth waveform while erasing extrema or introducing periodic content. This does not imply that waveform reconstruction is universally ineffective; it shows misalignment for the evaluated conventional methods, disturbances, recordings, representation, and downstream estimator.
...
\begin{figure}[t]
\centering
\begin{tikzpicture}[>=Latex,scale=0.84,transform shape,node distance=4.5mm and 5mm,
process/.style={draw,align=center,font=\scriptsize,minimum width=23mm,minimum height=7mm},
observation/.style={draw,double,align=center,font=\scriptsize,minimum width=23mm,minimum height=7mm},
arrow/.style={->,semithick}]
\node[process] (z) {Latent cardiac process $z(t)$};
\node[process,below left=of z] (el) {Electrical dynamics};
\node[process,below right=of z] (mech) {Mechanical dynamics};
\node[observation,below=of el] (ecg) {ECG observation};
\node[process,below=of mech] (transfer) {Body / support /\\sensor transfer};
\node[observation,below=of transfer] (bcg) {BCG observation};
...
A controlled future study should hold model capacity, training data, and evaluation conditions fixed while comparing waveform-reconstruction objectives with task-relevant or cross-modal representation objectives. ECG may provide teacher or reference cardiac information during research, whereas deployment should ultimately operate from BCG alone. Representation alignment should be constrained by clean-input preservation or bounded transformations so that agreement cannot be achieved through arbitrary waveform synthesis. Evaluation should use held-out verified continuous-source groups and report induced failures together with recovered failures and source-level heterogeneity. These principles define a research hypothesis rather than a commitment to a particular architecture or an assumption of improved performance.
...
The project includes deterministic provenance validation, fixed perturbation configurations, estimator instrumentation, paired window/source summaries, and scripts for aggregate figure generation. Publication figures contain aggregate results only; raw BCG/ECG recordings and personal-data-derived per-window outputs are excluded.
